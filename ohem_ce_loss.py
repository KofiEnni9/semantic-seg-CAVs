...
class OhemCELoss(nn.Module):
    ...
    def forward(self, logits, labels):
        # Ensure logits are in the correct shape (B, C, H, W)
        if logits.dim() == 4:
            n, c, h, w = logits.size()
        ...
        # Reshape logits if necessary (assuming B, C, H, W format)
        if logits.dim() == 4:
            n, c, h, w = logits.size()
            logits = logits.permute(0, 2, 3, 1).contiguous().view(-1, c)
            labels = labels.view(-1)
        ...
        n_min = valid_mask.sum() // 16

        if n_min > 0:
            loss_hard = valid_loss[valid_loss > self.thresh]
            if loss_hard.numel() < n_min:
                loss_hard, _ = valid_loss.topk(n_min)
